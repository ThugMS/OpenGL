...
void drawRobotArm(float ang1, float ang2, float ang3)
{
	...
	CTM = Translate(0, -0.4, 0) * RotateY(_time*5);
	M = Scale(0.3, 0.2, 0.3);
...
void myDisplay()
{
	...
	CTM = Translate(0, -0.4, 0) * RotateY(_time*30);
	drawRobotArm(ang1, ang2, ang3);	
	

	glUniform4f(uColor, 1,0,0,1);
	if(bDrawTarget == true)
		target.Draw(program, CTM, _time);
...
void myIdle()
{
	if(bPlay)
	{
		...
		{
			ang1 = 45 * sin(_time*3.141592);
			ang2 = 60 * sin(_time*2*3.141592);
			ang3 = 30 * sin(_time*3.141592);
		}
		else
			computeAngle();
