...
void myInitTexture()
{
	...
	//unsigned char img[iheight][iwidth][3];
	GLubyte img[iheight][iwidth][3];
	for (int y = 0; y < iheight; y++)
		for (int x = 0; x < iwidth; x++)
		{
			unsigned char val = 0;
			if ((x / 4 + y / 4) % 2 == 1) val = 255;
			else val = 0;
			img[y][x][0] = val;			// red

			img[y][x][1] = 0;			// green

			if (((x + 1) / 4 + (y + 1) / 4) % 2 == 1) val = 255;
			else val = 0;
			img[y][x][2] = val;			// blue
		}
	...
	for (int i = 0; i < 4; i++) {
		...
		switch (i) {
		case 0:
			img2.loadTGA("earthmap_dif.tga");
			glActiveTexture(GL_TEXTURE0);
			break;
		case 1:
			img2.loadTGA("earthmap_cld.tga");
			glActiveTexture(GL_TEXTURE1);
			break;

		case 2:
			img2.loadTGA("earthmap_spec.tga");
			glActiveTexture(GL_TEXTURE2);
			break;

		case 3:
			img2.loadTGA("earthmap_light.tga");
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img2.width, img2.height, 0,
			GL_BGR, GL_UNSIGNED_BYTE, img2.data);
...
void display()
{
	...
	GLuint uTexSpec = glGetUniformLocation(phong_prog, "uTexSpec");
	GLuint uTexLight = glGetUniformLocation(phong_prog, "uTexLight");
	...
	glUniform1i(uTexSpec, 2);
	glUniform1i(uTexLight, 3);
